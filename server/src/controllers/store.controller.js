...
const createStore = asyncHandler(async (req, res) => {
  ...
  let logoUrl = 'https://res.cloudinary.com/demo/image/upload/v1502432214/store-placeholder.png';
  if (req.file?.path) {
  ...
  const isApproved = req.user.role === ROLES.ADMIN;
  ...
  // Register vendor application queue for admin panels
  await VendorApplication.create({
    user: req.user._id,
    storeName: name.trim(),
    businessDescription: description,
    businessAddress: address ? `${address.street}, ${address.city}, ${address.state}, ${address.postalCode}, ${address.country}` : 'Not Specified',
    phone,
